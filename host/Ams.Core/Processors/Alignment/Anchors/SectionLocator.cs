...
public static class SectionLocator
{
    ...
    public static SectionRange? DetectSection(BookIndex book, IReadOnlyList<string> asrTokens, int prefixTokenCount = 8)
    {
        ...
        var asrNormToks = new List<string>(Math.Max(1, prefixTokenCount));
        TextNormalizer.TokenizeWords(asrNorm, asrNormToks);
        if (asrNormToks.Count == 0) return null;
        var asrFirstToken = asrNormToks[0];
        var asrFirstIsHeading = HeadingKeywords.Contains(asrFirstToken);

        // Compute a simple prefix-match score against each section title
        SectionRange? best = null;
        int bestScore = 0;
        var titleToks = new List<string>(8);
        foreach (var sec in book.Sections)
        {
            ...
            titleToks.Clear();
            TextNormalizer.TokenizeWords(titleNorm, titleToks);
            if (titleToks.Count == 0) continue;

            int lcp = LongestCommonPrefix(asrNormToks, titleToks);
            // Heuristic boost: if both start with a heading keyword (e.g., "chapter").
            if (asrFirstIsHeading && titleToks[0] == asrFirstToken)
            {
        ...
        bool isHeadingKeyword = asrFirstIsHeading;
