...
public static class AudioIntegrityVerifier
{
        ...
        double minDeltaDb = 20.0)
    {
        if (sampleRateRaw != sampleRateTreated)
            throw new InvalidOperationException(
                $"Sample rate mismatch: raw={sampleRateRaw}, treated={sampleRateTreated}");
        ...
        var missing = CollectRuns(missingMask, stepSec, windowSec, minMismatchMs / 1000.0,
    ...
    private static List<Segment> CollectRuns(bool[] mask, double stepSec, double windowSec, double minDurSec,
        AudioMismatchType type)
    {
