...
Welcome to Stage 24! In this stage, you'll learn about the advanced WebAuthn implementation in the Ams.Web project, which enables secure passwordless authentication using biometric methods like fingerprint, Face ID, and PIN across all platforms.

---

## 🔐 What is WebAuthn?

**WebAuthn** (Web Authentication) is a modern web standard that enables passwordless authentication using public-key cryptography. The Ams.Web project implements a comprehensive WebAuthn solution that works across:
...
**`WebAuthnCredential` Entity** (`Ams.Web.Server.Api/Models/Identity/WebAuthnCredential.cs`):
...
**`WebAuthnServiceBase`** (`Ams.Web.Client.Core/Services/WebAuthnServiceBase.cs`):
...
**`ILocalHttpServer`** Interface (`Ams.Web.Client.Core/Services/Contracts/ILocalHttpServer.cs`):
...
**Location**: `Ams.Web.Client.Web/wwwroot/web-interop-app.html`
...
{
    options.ServerName = "Ams.Web WebAuthn";
