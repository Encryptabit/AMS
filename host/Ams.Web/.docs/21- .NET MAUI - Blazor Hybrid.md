...
4. [Ams.Web.Client.Windows Advantages](#ams.webclientwindows-advantages) - Better Windows support
...
The `Ams.Web.Client.Maui` and `Ams.Web.Client.Windows` projects demonstrate this architecture in action.
...
**Location**: [`/src/Client/Ams.Web.Client.Maui/Platforms/Android/MainActivity.cs`](/src/Client/Ams.Web.Client.Maui/Platforms/Android/MainActivity.cs)
...
#### iOS - Universal Links
For iOS, this is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `Ams.Web.Client.Web` project. No manual configuration is needed per page!

**Location**: [`/src/Client/Ams.Web.Client.Web/wwwroot/apple-app-site-association`](/src/Client/Ams.Web.Client.Web/wwwroot/apple-app-site-association)
...
### ApplicationVersion in Ams.Web.Client.Maui

The `ApplicationVersion` property in the `Ams.Web.Client.Maui.csproj` file manages the app's version for store submissions and updates.
...
## Ams.Web.Client.Windows Advantages

The `Ams.Web.Client.Windows` project offers several advantages over the MAUI Windows output:
...
The app checks the WebView version on Android to ensure compatibility in [`App.xaml.cs`](/src/Client/Ams.Web.Client.Maui/App.xaml.cs):
...
The MAUI project is located at: `src/Client/Ams.Web.Client.Maui/`
...
The [`MauiProgram.cs`](/src/Client/Ams.Web.Client.Maui/MauiProgram.cs) file is the starting point of the MAUI application:
...
Services specific to MAUI are registered in [`MauiProgram.Services.cs`](/src/Client/Ams.Web.Client.Maui/MauiProgram.Services.cs):
...
The [`MainPage.xaml`](/src/Client/Ams.Web.Client.Maui/MainPage.xaml) file hosts the `BlazorWebView` component, which renders Blazor content inside a native WebView:
...
- It renders the `Routes` component (from `Ams.Web.Client.Core`) inside the `#app-container` element
- All your Blazor components render inside this WebView, just like in the browser!

### 4. BlazorWebView Customization

The `SetupBlazorWebView()` method in [`MauiProgram.cs`](/src/Client/Ams.Web.Client.Maui/MauiProgram.cs) customizes the WebView for each platform:
...
Implements `IStorageService` using MAUI's `Preferences` API in [`MauiStorageService.cs`](/src/Client/Ams.Web.Client.Maui/Services/MauiStorageService.cs):
...
Implements `IExternalNavigationService` for opening external URLs in [`MauiExternalNavigationService.cs`](/src/Client/Ams.Web.Client.Maui/Services/MauiExternalNavigationService.cs):
