...
#### Server API Resources (`src/Server/Ams.Web.Server.Api/Resources/`)
...
The `AppDataAnnotationsValidator` (located in `src/Client/Ams.Web.Client.Core/Components/AppDataAnnotationsValidator.cs`) extends the validation system to:
...
Here's an actual example from the project (`src/Client/Ams.Web.Client.Core/Components/Pages/Categories/AddOrEditCategoryModal.razor`):
...
**Base class location**: `src/Client/Ams.Web.Client.Core/Components/AppComponentBase.cs`
...
**Base class location**: `src/Server/Ams.Web.Server.Api/Controllers/AppControllerBase.cs`
