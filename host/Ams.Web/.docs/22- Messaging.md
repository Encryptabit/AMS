...
Welcome to Stage 22! In this stage, you will learn about the comprehensive **messaging and real-time communication system** built into the Ams.Web project. This system provides a unified messaging architecture that enables communication across multiple channels and platforms.
...
At the heart of the Ams.Web messaging architecture is **AppMessages** - a centralized messaging system that provides a consistent way to communicate across different parts of your application, regardless of whether the communication happens:
...
**Location**: [`src/Shared/Services/ClientAppMessages.cs`](/src/Client/Ams.Web.Client.Core/Services/ClientAppMessages.cs)
...
**Location**: [`src/Client/Ams.Web.Client.Core/Services/ClientAppMessages.cs`](/src/Client/Ams.Web.Client.Core/Services/ClientAppMessages.cs)
...
The Ams.Web project provides multiple communication channels that all work with the same centralized message system. Let's explore each channel and understand when to use them.
...
**Location**: [`src/Client/Ams.Web.Client.Core/Services/PubSubService.cs`](/src/Client/Ams.Web.Client.Core/Services/PubSubService.cs)
...
**SignalR** enables the server to send messages to clients in real-time. In the Ams.Web project, SignalR messages are automatically bridged to PubSubService, creating a seamless experience.

**Server-Side Hub**: [`src/Server/Ams.Web.Server.Api/SignalR/AppHub.cs`](/src/Server/Ams.Web.Server.Api/SignalR/AppHub.cs)
...
**Location**: [`src/Client/Ams.Web.Client.Core/Components/Layout/AppJsBridge.razor.cs`](/src/Client/Ams.Web.Client.Core/Components/Layout/AppJsBridge.razor.cs)
...
The `window.postMessage` API allows communication between different JavaScript contexts (e.g., iframes, service workers). The Ams.Web project bridges this to PubSubService.

**Location**: [`src/Client/Ams.Web.Client.Core/Scripts/events.ts`](/src/Client/Ams.Web.Client.Core/Scripts/events.ts)
...
**Location**: [`src/Client/Ams.Web.Client.Web/wwwroot/service-worker.js`](/src/Client/Ams.Web.Client.Web/wwwroot/service-worker.js)
...
**Location**: [`src/Client/Ams.Web.Client.Core/Components/AppClientCoordinator.cs`](/src/Client/Ams.Web.Client.Core/Components/AppClientCoordinator.cs)
...
The Ams.Web project supports **push notifications** across all platforms:
...
**Client-Side Interface**: [`src/Client/Ams.Web.Client.Core/Services/Contracts/IPushNotificationService.cs`](/src/Client/Ams.Web.Client.Core/Services/Contracts/IPushNotificationService.cs)

**Base Implementation**: [`src/Client/Ams.Web.Client.Core/Services/PushNotificationServiceBase.cs`](/src/Client/Ams.Web.Client.Core/Services/PushNotificationServiceBase.cs)
...
**Location**: [`src/Server/Ams.Web.Server.Api/Services/PushNotificationService.cs`](/src/Server/Ams.Web.Server.Api/Services/PushNotificationService.cs)
...
**Extension Helper**: [`src/Client/Ams.Web.Client.Core/Extensions/NotificationExtensions.cs`](/src/Client/Ams.Web.Client.Core/Extensions/NotificationExtensions.cs)
...
When testing push notifications, it's critical to understand that there are **four distinct scenarios** based on the app state when the notification is sent and when the user taps on it. The Ams.Web project handles all four scenarios across all platforms.
...
* [Describe the workflow of bit Ams.Web's AI chat feature and provide a high-level overview.
