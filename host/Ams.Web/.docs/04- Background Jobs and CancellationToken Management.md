...
Welcome to **Stage 4** of the Ams.Web project tutorial! In this stage, we'll explore how the project handles **cancellation tokens** for request cancellation and **background job processing** with Hangfire.
...
Let's look at a real controller from the project - [`TodoItemController.cs`](/src/Server/Ams.Web.Server.Api/Controllers/Todo/TodoItemController.cs):
...
Let's examine how `CurrentCancellationToken` is implemented in [`AppComponentBase.cs`](/src/Client/Ams.Web.Client.Core/Components/AppComponentBase.cs):
...
Here's a real example from [`TodoPage.razor.cs`](/src/Client/Ams.Web.Client.Core/Components/Pages/TodoPage.razor.cs):
...
In [`PhoneService.cs`](/src/Server/Ams.Web.Server.Api/Services/PhoneService.cs):
...
In [`PhoneServiceJobsRunner.cs`](/src/Server/Ams.Web.Server.Api/Services/Jobs/PhoneServiceJobsRunner.cs):
...
In [`Program.Services.cs`](/src/Server/Ams.Web.Server.Api/Program.Services.cs):
