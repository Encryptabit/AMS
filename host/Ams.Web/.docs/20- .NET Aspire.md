...
Welcome to Stage 20! In this stage, you'll learn about .NET Aspire and how it transforms both development and deployment experiences in this Ams.Web project.
...
- **Built-in Development Tools**: 
  ...
  - **Azurite (Azure Storage emulator)** for blob storage development
...
**Location**: [`src/Server/Ams.Web.Server.AppHost/`](/src/Server/Ams.Web.Server.AppHost/)
...
// Azure Storage emulator (Azurite) for blob development
...
serverWebProject.WithReference(blobStorage);
...
2. **Azure Storage Emulator**: Azurite keeps blob data local while mirroring Azure Blob Storage semantics
3. **DbGate Integration**: Web-based database management UI accessible during development
4. **Data Volumes & Dependencies**: Persistent volumes plus `WaitFor` ensure infrastructure is ready before Server.Web starts
5. **Conditional Services**: MailPit and the WebAssembly client only run in Aspire's run mode
...
**Location**: [`src/Server/Ams.Web.Server.AppHost/appsettings.Development.json`](/src/Server/Ams.Web.Server.AppHost/appsettings.Development.json)
...
- Uses the default `UseDevelopmentStorage=true` connection string provisioned by Aspire
- Persists blob data on a Docker volume through `.WithDataVolume()`
...
- ❌ `Ams.Web.Server.Api` **does NOT** run on Docker
- ❌ `Ams.Web.Server.Web` **does NOT** run on Docker
...
3. Run this command in the terminal to clean previous Windows build artifacts:
   ...
   ```
4. Then run the `Ams.Web.Server.AppHost` project
