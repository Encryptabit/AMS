...
Welcome to **Stage 7** of the Ams.Web project tutorial! In this stage, you will explore the comprehensive **authentication and authorization system** built into the project. This production-ready identity system includes JWT-based authentication, role and permission management, session handling, two-factor authentication, and much more.
...
- **Security**: Tokens are signed using a secret key configured in [`appsettings.json`](/src/Server/Ams.Web.Server.Api/appsettings.json)

#### Token Configuration

You can configure token expiration in [`appsettings.json`](/src/Server/Ams.Web.Server.Api/appsettings.json):
...
User sessions are persisted in the database through the [`UserSession`](/src/Server/Ams.Web.Server.Api/Models/Identity/UserSession.cs) entity:
...
From [`IdentityController.cs`](/src/Server/Ams.Web.Server.Api/Controllers/Identity/IdentityController.cs):
...
By default, the project is configured to connect to a **demo Duende Identity Server 8 instance** to demonstrate how external OpenID Connect providers work. The implementation is in [`IdentityController.SocialSignIn.cs`](/src/Server/Ams.Web.Server.Api/Controllers/Identity/IdentityController.SocialSignIn.cs).

#### Configuration

External provider settings are configured in [`appsettings.json`](/src/Server/Ams.Web.Server.Api/appsettings.json) under the `Authentication` section:
...
From [`IdentityController.SocialSignIn.cs`](/src/Server/Ams.Web.Server.Api/Controllers/Identity/IdentityController.SocialSignIn.cs):
...
**Location**: [`/src/Server/Ams.Web.Server.Api/appsettings.json`](/src/Server/Ams.Web.Server.Api/appsettings.json)
...
Each token type has a corresponding `RequestedOn` timestamp in the [`User`](/src/Server/Ams.Web.Server.Api/Models/Identity/User.cs) entity:
...
From [`IdentityController.EmailConfirmation.cs`](/src/Server/Ams.Web.Server.Api/Controllers/Identity/IdentityController.EmailConfirmation.cs):
...
From [`IdentityController.ResetPassword.cs`](/src/Server/Ams.Web.Server.Api/Controllers/Identity/IdentityController.ResetPassword.cs):
...
* [How does a `refresh token` function in a Ams.Web project template?](https://deepwiki.com/search/how-does-a-refresh-token-funct_6a75fa66-ab98-4367-bd1a-24b081fbf88c)
