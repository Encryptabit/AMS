...
- **Migrations (Optional)**: EF Core migrations is **not mandatory**, especially for test projects or rapid prototyping scenarios where the database can be recreated easily.
  ...
  - **How to Switch to Migrations**:
    1. Replace `Database.EnsureCreatedAsync()` with `Database.MigrateAsync()` in the following 3 files so the developer would understand where to make the changes:
       1. [/src/Server/Ams.Web.Server.Api/Program.cs](/src/Server/Ams.Web.Server.Api/Program.cs)
       2. [/src/Server/Ams.Web.Server.Web/Program.cs](/src/Server/Ams.Web.Server.Web/Program.cs)
    ...
    3. Create your first migration for server side `AppDbContext` by running `dotnet ef migrations add Initial --output-dir Data/Migrations --verbose` within the `Ams.Web.Server.Api` directory
...
- **Details**: Read provided information in `/src/Server/Ams.Web.Server.Api/Mappers/Readme.md`
...
- **Mapper Files**: Explain 1 mapper file written in `Ams.Web.Server.Api` project using Mapperly (e.g., `CategoriesMapper`, `IdentityMapper` etc)
...
- Mapperly usage in Ams.Web.Client.Core: Try finding `.Patch` usages in `Ams.Web.Client.Core` project and explain the way mapperly is used for patching dto objects returned from server to update existing dto objects in client memory without replacing the whole object reference.
...
- **Proxy Interface**: Explain how interfaces are defined in `Shared/Controllers` and implemented in `Ams.Web.Server.Api/Controllers` using provided information in [/src/Shared/Controllers/Readme.md](/src/Shared/Controllers/Readme.md)
...
- **Search**: Locate `PhoneServiceJobsRunner.cs` in the `Ams.Web.Server.Api` project and its usages and explain it to the developer.
...
- **Resx Files**: Explain the resource files structure:
  - **Ams.Web.Shared Project**: `AppStrings.resx` and `IdentityStrings.resx` for UI strings
  - **Ams.Web.Server.Api Project**: `EmailStrings.resx` for email templates
...
- **Location**: Read [/src/Server/Ams.Web.Server.Api/appsettings.json](/src/Server/Ams.Web.Server.Api/appsettings.json)
...
1. **Find an example page** from the project (e.g., a page from `src/Client/Ams.Web.Client.Core/Components/Pages/`)
...
3. **Show actual examples** from the chosen page:
   ...
   - **Important** How `AboutPage.razor` has been added to `Ams.Web.Client.Maui`, `Ams.Web.Client.Windows` and `Ams.Web.Client.Web` projects for to have access to native platform features without using interface and dependency injection,
...
#### Global Styles
- **app.scss**: Read the main global stylesheet located in [/src/Client/Ams.Web.Client.Core/Styles/app.scss](/src/Client/Ams.Web.Client.Core/Styles/app.scss)
...
2. Understand the configuration priority/hierarchy from `IConfigurationBuilderExtensions.cs` in `Ams.Web.Client.Core`.
...
### Instructions
1. Show `tsconfig.json` and `package.json` from `src/Client/Ams.Web.Client.Core/`
2. Explain MSBuild targets in `Ams.Web.Client.Core.csproj`: `BeforeBuildTasks` → `InstallNodejsDependencies` → `BuildJavaScript`
...
1. **Find and show these 3 files**:
   - `Ams.Web.Server.Web/Components/App.razor`: For Blazor Server, Auto, and WebAssembly with pre-rendering
   - `Ams.Web.Client.Web/wwwroot/index.html`: For standalone Blazor WebAssembly
   - `Ams.Web.Client.Maui/wwwroot/index.html`: For Blazor Hybrid (also used by Client.Windows)
...
- **Location**: `Ams.Web.Server.Api/appsettings.json` under `WebAppRender` section
- **Settings**:
...
- **If you enable PreRendering**, update `Ams.Web.Client.Web/wwwroot/service-worker.published.js` accordingly
...
3. Show how server validates version via middleware against `SupportedAppVersions` in `Ams.Web.Server.Api/appsettings.json`
...
### Instructions
   ...
   - [/src/Server/Ams.Web.Server.Shared/Services/AppResponseCachePolicy.cs](/src/Server/Ams.Web.Server.Shared/Services/AppResponseCachePolicy.cs)
   - [/src/Server/Ams.Web.Server.Api/Services/ResponseCacheService.cs](/src/Server/Ams.Web.Server.Api/Services/ResponseCacheService.cs)
   - [/src/Client/Ams.Web.Client.Core/Services/HttpMessageHandlers/CacheDelegatingHandler.cs](/src/Client/Ams.Web.Client.Core/Services/HttpMessageHandlers/CacheDelegatingHandler.cs)
...
3. **Important Database Management Tip**:
   ...
   - **The Solution**: To keep the database running persistently:
     1. Open `Program.cs` in the `Ams.Web.Server.AppHost` project
   ...
   - **Credentials**: The credentials are stored in `appsettings.Development.json` in `Ams.Web.Server.AppHost` project.

4. **Important Note about Docker Execution**:
   - **For Performance**: Aspire does NOT run the `Ams.Web.Server.Api` and `Ams.Web.Server.Web` projects on Docker (they run natively for faster development)
...
1. **Explain the native platform projects**:
   - **Overview**: The `Ams.Web.Client.Windows` and `Ams.Web.Client.Maui` projects provide native outputs for Android, iOS, macOS, and Windows with full access to operating system features
   - **Key Benefits**:
...
2. **Explain Deep Links (Android) and Universal Links (iOS)**:
   ...
   - **Configuration Requirements**:
     ...
     - **iOS**: This is handled automatically by the `apple-app-site-association` file in the `wwwroot` of the `Ams.Web.Client.Web` project - no manual configuration needed per page
   - **Important Setup**:
...
3. **Explain ApplicationVersion in Ams.Web.Client.Maui**: Explain that the `ApplicationVersion` property in the `Ams.Web.Client.Maui.csproj` file is used to manage the app's version for store submissions and updates.
...
4. **Explain Ams.Web.Client.Windows advantages**:
 - **Broader OS Support**: The Windows output works on Windows 7, 8, 10, and 11
  - In contrast, `Ams.Web.Client.Maui`'s Windows output only works on Windows 10 and 11
   - **Better Performance**: `Ams.Web.Client.Windows` is faster than the MAUI Windows version
...
2. **Explain features**
   ...
   - The developer can publish a message to navigate to specific page when a user taps on a push notification `Ams.Web.Client.Web/wwwroot/service-worker.js`
...
3. **Explain Embedding Models**:
   ...
   - **Configuration**: These models are configured in `appsettings.json` under the `AI` section in `Ams.Web.Server.Api` project

4. **Enable Embeddings in the Project**:
   - **Step 1**: Open `AppDbContext.cs` in `Ams.Web.Server.Api/Data` folder
   - **Step 2**: Find the `IsEmbeddingEnabled` static property and change it from `false` to `true`.

5. **Find and Show Implementation**:
   - Search for and explain `ProductEmbeddingService.cs` in the `Ams.Web.Server.Api/Services` folder
