...
*   **Ams.Web.Server.Api**: Houses API controllers, mappers, the `DbContext`, EF Core migrations, email templates, action filters, SignalR hubs, and server-specific configuration.
*   **Ams.Web.Server.Web**: The application's default startup project and entry point. It hosts `App.razor` and configures Blazor Server and server-side rendering (SSR).
*   **Ams.Web.Server.Shared**: (Also known as Aspire's ServiceDefaults) Contains common code shared between the `Ams.Web.Server.Api` and `Ams.Web.Server.Web` projects.
*   **Ams.Web.Server.AppHost**: Manages the .NET Aspire configuration and orchestration.
*   **Ams.Web.Shared**: Contains shared DTOs, enums, custom exceptions, shared services, and `.resx` resource files.
*   **Ams.Web.Tests**: Contains all UI and integration tests.
*   **Ams.Web.Client.Core**: The heart of the client application. Contains all shared Blazor components, pages, layouts, client-side services, and the primary `App.ts` and `App.scss` files.
*   **Ams.Web.Client.Web**: The Blazor WebAssembly (WASM) standalone project.
*   **Ams.Web.Client.Maui**: The .NET MAUI Blazor Hybrid project for native mobile and desktop apps.
*   **Ams.Web.Client.Windows**: The Windows Forms Blazor Hybrid project.
...
-   **Build the project**: Run `dotnet build` in Ams.Web.Server.Web project root directory.
-   **Run the project**: Run `dotnet run` in Ams.Web.Server.Web project root directory.
-   **Run tests**: Run `dotnet test` in Ams.Web.Tests project root directory.
-   **Add new migrations**: Run `dotnet ef migrations add <MigrationName> --output-dir Data/Migrations --verbose` in Ams.Web.Server.Api project root directory.
-   **Generate Resx C# code**: Run `dotnet build -t:PrepareResources` in Ams.Web.Shared project root directory (It's automatically done during build).
...
- **Location**: `Ams.Web.Server.Api's Models folder`
- **Requirements**:
...
- **Location**: `Ams.Web.Server.Api's Data/Configuration folder`
...
Create the DTO
- **Location**: `Ams.Web.Shared's Dtos folder`
- **Requirements**:
...
Create the Mapper
- **Location**: `Ams.Web.Server.Api's Mappers folders`
- **Requirements**:
...
- **Location**: `Ams.Web.Shared project's Controllers folder`
- **Requirements**:
...
- **Location**: `Ams.Web.Server.Api's Controllers folder`
- **Requirements**:
