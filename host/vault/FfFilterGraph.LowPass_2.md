---
namespace: "Ams.Core.Services.Integrations.FFmpeg"
project: "Ams.Core"
source_file: "Projects/AMS/host/Ams.Core/Services/Integrations/FFmpeg/FfFilterGraph.cs"
access_modifier: "public"
complexity: 1
fan_in: 0
fan_out: 1
tags:
  - method
  - llm/utility
  - llm/factory
  - llm/validation
---
# FfFilterGraph::LowPass
**Path**: `Projects/AMS/host/Ams.Core/Services/Integrations/FFmpeg/FfFilterGraph.cs`

## Summary
**Add a low-pass filter to the `FfFilterGraph` using simple numeric arguments.**

This expression-bodied overload is a convenience wrapper that maps scalar inputs to a `LowPassFilterParams` instance and delegates to `LowPass(LowPassFilterParams?)`. It applies default values of `frequencyHz = 12000` and `poles = 2`, keeping the fluent API concise while centralizing actual filter construction and validation in the parameterized overload.


#### [[FfFilterGraph.LowPass_2]]
##### What it does:
<member name="M:Ams.Core.Services.Integrations.FFmpeg.FfFilterGraph.LowPass(System.Double,System.Double)">
    <summary>
    Low-pass filter (libavfilter <c>lowpass</c>).
    </summary>
</member>

##### Improvements:
- _TODO: Suggested optimizations._

```csharp
public FfFilterGraph LowPass(double frequencyHz = 12000, double poles = 2)
```

**Calls ->**
- [[FfFilterGraph.LowPass]]

