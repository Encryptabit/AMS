...
## Summary
**Provides the source-generated regex instance used to identify section-heading text prefixes.**

`MyRegex()` is a `[GeneratedRegex]` partial factory method that emits a compiled, case-insensitive `Regex` for detecting section-heading prefixes at line start. The pattern `^\s*(chapter\b|prologue\b|epilogue\b|prelude\b|foreword\b|introduction\b|afterword\b|appendix\b)` matches optional leading whitespace followed by one of the canonical heading keywords, with culture set to `"en-US"`. It is used to initialize the static `SectionTitleRegex` matcher.
